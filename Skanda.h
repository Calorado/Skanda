...
namespace skanda {
	...
	LZStructure<IntType>* skanda_forward_optimal_parse(const uint8_t* const input, const uint8_t* const inputStart,
		...
		if (lastMatchLength) {
			...
			const uint8_t* inputPosition = matchEnd - std::min((size_t)compressorOptions.niceLength / 4, lastMatchLength) + 1;
